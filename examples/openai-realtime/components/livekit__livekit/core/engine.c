...
static bool establish_peer_connections(engine_t *eng, const livekit_pb_join_response_t *join)
{
    ...
    ESP_LOGI(TAG, "Creating publisher peer... Free heap: %lu, SPIRAM: %lu",
             (unsigned long)esp_get_free_heap_size(),
             (unsigned long)heap_caps_get_free_size(MALLOC_CAP_SPIRAM));
    _create_and_connect_peer(&options, &eng->pub_peer_handle);
    if (eng->pub_peer_handle == NULL)
        return false;
    ESP_LOGI(TAG, "Publisher peer created. Free heap: %lu, SPIRAM: %lu",
             (unsigned long)esp_get_free_heap_size(),
             (unsigned long)heap_caps_get_free_size(MALLOC_CAP_SPIRAM));

    // Small delay to let publisher peer fully initialize before creating subscriber
    ...
    ESP_LOGI(TAG, "Creating subscriber peer... Free heap: %lu, SPIRAM: %lu",
             (unsigned long)esp_get_free_heap_size(),
             (unsigned long)heap_caps_get_free_size(MALLOC_CAP_SPIRAM));
