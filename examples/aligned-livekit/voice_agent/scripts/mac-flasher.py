...
"""
Mac ESP32-S3 Flasher using libusb for WCH CH343 USB-Serial chips
Bypasses the macOS CDC driver by using libusb directly with conservative
packet sizes (16 B chunks + 5 ms inter-chunk delay).

NOTE (2026-05-18): This is the SLOW failsafe path (~3 KB/s). For normal
flashing use ../scripts/esptool_small_blocks.py — it goes through the
macOS CDC driver at 252 B blocks / 460800 baud and runs ~60× faster
(~190 kbit/s, ~5:27 for a 7.8 MB image). Only fall back to this script
if you suspect CDC driver issues or cable corruption.

The CH343's actual USB OUT max packet size is 32 B (verified via libusb
descriptor read). This script chunks at 16 B for extra margin.
...
# WCH CH343 USB identifiers (used in SenseCap Watcher)
WCH_VID = 0x1A86
WCH_PID = 0x55D2
...
# Constants
# CRITICAL: The WCH CH343 USB-Serial chip corrupts large packets.
...
class WCHFlasher:
    """ESP32-S3 flasher for WCH CH343 USB-Serial"""
    ...
    def find_device(self) -> bool:
        cprint(Colors.YELLOW, f"Looking for WCH CH343 (VID={hex(WCH_VID)}, PID={hex(WCH_PID)})...")
        self.dev = usb.core.find(idVendor=WCH_VID, idProduct=WCH_PID)
        if self.dev is None:
            cprint(Colors.RED, "Device not found!")
            return False
        cprint(Colors.GREEN, f"Found: {self.dev.product or 'WCH CH343'}")
        return True
    ...
    def write(self, data: bytes, timeout: int = DEFAULT_TIMEOUT):
        ...
        The WCH CH343 chip has a 32-byte USB OUT buffer. Sending data too fast
        ...
        while offset < total_len:
            ...
            # The WCH CH343 has limited internal buffering - if we send too fast,
...
def main():
    parser = argparse.ArgumentParser(
        description='Mac ESP32-S3 Flasher for WCH CH343 USB-Serial',
...
"""
    ...
    cprint(Colors.GREEN, "  Mac ESP32-S3 Flasher (WCH CH343)")
