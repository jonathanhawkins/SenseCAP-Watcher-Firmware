...
static int open_afe_in_ram(void *arg)
{
    ...
    do {
        ...
        // Disable wake word detection for always-on mic streaming.
        if (afe_config) {
...
static void log_mic_level(const int16_t *samples, int sample_count)
{
    ...
    int32_t peak_pct = (peak * 100) / 32767;
    int32_t avg_pct = (avg * 100) / 32767;
    ESP_LOGI(TAG, "Mic level peak=%" PRId32 "%% avg=%" PRId32 "%%", peak_pct, avg_pct);
}
...
{
    log_mic_level((const int16_t *)feed_data, feed_size / (int)sizeof(int16_t));
...
static inline int audio_aec_src_read_directly(audio_aec_src_t *src)
{
    ...
    do {
        ...
        while (!src->stopping) {
            ...
            log_mic_level((const int16_t *)feed_data, read_size / (int)sizeof(int16_t));
