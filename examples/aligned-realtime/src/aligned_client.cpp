/**
 * Aligned Tools Client for SenseCAP Watcher
 * 
 * Handles device authentication and LiveKit connection setup.
 * This replaces the direct OpenAI connection with Aligned's API.
 */
...
// WHIP-specific credentials (for ESP32 WebRTC via HTTP)
static char g_whip_url[512] = {0};
static char g_whip_token[256] = {0};

// Connection state tracking
static bool g_aligned_connected = false;

// HTTP response buffer (keep off stack to avoid console_repl stack overflow)
...
static esp_err_t aligned_load_token_from_nvs(void) {
    ...
    if (err != ESP_OK) {
        ESP_LOGW(LOG_TAG, "NVS namespace not found, token not set");
        return err;
    }
    ...
    if (err == ESP_OK) {
        ESP_LOGI(LOG_TAG, "Loaded device token from NVS (length: %d)", strlen(g_aligned_device_token));
    } else {
        ESP_LOGW(LOG_TAG, "No device token in NVS");
    }
...
static esp_err_t aligned_save_token_to_nvs(const char *token) {
    ...
    if (err != ESP_OK) {
        ESP_LOGE(LOG_TAG, "Failed to open NVS: %s", esp_err_to_name(err));
        return err;
    }
    
    err = nvs_set_str(handle, ALIGNED_NVS_TOKEN_KEY, token);
    if (err != ESP_OK) {
        ESP_LOGE(LOG_TAG, "Failed to save token: %s", esp_err_to_name(err));
        nvs_close(handle);
        return err;
    }
    ...
    if (err == ESP_OK) {
        ESP_LOGI(LOG_TAG, "Device token saved to NVS");
    }
...
void aligned_set_device_token(const char *token) {
    if (token == NULL || strlen(token) == 0) {
        ESP_LOGE(LOG_TAG, "Invalid token");
        return;
    }
    ...
    ESP_LOGI(LOG_TAG, "Device token set (length: %d)", strlen(token));
}
...
static esp_err_t aligned_load_api_url_from_nvs(void) {
    ...
    if (err == ESP_OK && strlen(g_aligned_api_url) > 0) {
        ESP_LOGI(LOG_TAG, "Using custom API URL: %s", g_aligned_api_url);
    }
...
void aligned_set_api_url(const char *url) {
    ...
    if (url == NULL || strlen(url) == 0) {
        ...
        ESP_LOGI(LOG_TAG, "API URL reset to default: %s", ALIGNED_API_BASE_DEFAULT);
    } else {
        ...
        ESP_LOGI(LOG_TAG, "API URL set to: %s", g_aligned_api_url);
    }
...
esp_err_t aligned_get_livekit_credentials(void) {
    if (!aligned_has_token()) {
        ESP_LOGE(LOG_TAG, "No device token configured. Use 'aligned_token -t <token>' to set.");
    ...
    ESP_LOGI(LOG_TAG, "Requesting LiveKit credentials from %s...", api_base);
    ...
    config.url = url;
    config.event_handler = aligned_http_event_handler;
    config.user_data = g_http_response_buffer;
    ...
    if (status_code != 200) {
        ESP_LOGE(LOG_TAG, "API returned error status: %d", status_code);
        ESP_LOGE(LOG_TAG, "Response: %s", g_http_response_buffer);
        esp_http_client_cleanup(client);
        return ESP_FAIL;
    }
    ...
    if (!cJSON_IsTrue(success)) {
        ESP_LOGE(LOG_TAG, "API returned success=false");
    ...
    cJSON *whip_url = cJSON_GetObjectItem(response, "whip_url");
    cJSON *whip_token = cJSON_GetObjectItem(response, "whip_token");
    ...
    // WHIP credentials for ESP32 WebRTC connection
    if (whip_url && cJSON_IsString(whip_url)) {
        strncpy(g_whip_url, whip_url->valuestring, sizeof(g_whip_url) - 1);
    }
    if (whip_token && cJSON_IsString(whip_token)) {
        strncpy(g_whip_token, whip_token->valuestring, sizeof(g_whip_token) - 1);
    }

    cJSON_Delete(response);

    ESP_LOGI(LOG_TAG, "✅ LiveKit credentials received:");
    ESP_LOGI(LOG_TAG, "   - URL: %s", g_livekit_url);
    ESP_LOGI(LOG_TAG, "   - Room: %s", g_room_name);
    ESP_LOGI(LOG_TAG, "   - Voice Provider: %s ($0.05/min)", g_voice_provider);
    ESP_LOGI(LOG_TAG, "   - Session: %s", g_session_id);
    if (strlen(g_whip_url) > 0) {
        ESP_LOGI(LOG_TAG, "   - WHIP URL: %s", g_whip_url);
        ESP_LOGI(LOG_TAG, "   - WHIP Token: %s...", g_whip_token);
    } else {
        ESP_LOGW(LOG_TAG, "   - WHIP URL: not provided (will construct from livekit_url)");
    }

    // Mark as connected so livekit_webrtc() knows we have credentials
...
/**
 * Get WHIP URL for direct HTTP WebRTC connection (ESP32)
 * Returns NULL if not provided by server
 */
const char* aligned_get_whip_url(void) {
    return strlen(g_whip_url) > 0 ? g_whip_url : NULL;
}

/**
 * Get WHIP stream key/token
 * Returns NULL if not provided by server
 */
const char* aligned_get_whip_token(void) {
    return strlen(g_whip_token) > 0 ? g_whip_token : NULL;
}
