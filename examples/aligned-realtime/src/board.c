...
#include "board.h"
#include "iot_button.h"        // For button_config_t
#include "iot_knob.h"          // For knob_config_t
#include <stdbool.h>

// Knob button functions from sensecap-watcher component
extern esp_err_t bsp_knob_btn_init(void *param);
extern esp_err_t bsp_knob_btn_deinit(void *param);
extern uint8_t bsp_knob_btn_get_key_value(void *param);

// Knob encoder GPIO pins (from sensecap-watcher.h)
...
#define DRV_LCD_PIXEL_CLK_HZ      (10 * 1000 * 1000)  // 10MHz - matches stable livekit config, reduces SPI queue errors
...
#define CONFIG_LVGL_DRAW_BUFF_HEIGHT (40)
#endif
#define LVGL_DRAW_BUFF_HEIGHT (CONFIG_LVGL_DRAW_BUFF_HEIGHT)  // Reduced from 412 to 40 to ease SPI load
...
static esp_err_t bsp_i2c0_bus_init(void)
{
    ...
    // --- Legacy Init Recreation: Silence LCD/SPI buses ---
    // IMPORTANT: Do NOT touch GPIO38/39 (I2C1 touch bus) here!
    // The I2C driver must configure these pins itself for open-drain mode.
    // Driving them as push-pull outputs breaks the I2C bus.
    ...
    // Configure LCD/SPI pins only - exclude I2C1 pins (GPIO38/39) and BACKLIGHT (GPIO8)
    // IMPORTANT: Do NOT touch GPIO8 (backlight) here - it's controlled by LEDC PWM after LCD init
    const gpio_config_t io_config = {
        .pin_bit_mask = (1ULL << BSP_SPI3_HOST_PCLK) | (1ULL << BSP_SPI3_HOST_DATA0) | (1ULL << BSP_SPI3_HOST_DATA1)
                        | (1ULL << BSP_SPI3_HOST_DATA2) | (1ULL << BSP_SPI3_HOST_DATA3) | (1ULL << BSP_LCD_SPI_CS),
    ...
    // ESP_FAIL means driver already installed by sensecap-watcher's bsp_i2c_bus_init()
    // This is OK - we just need to track the port number so subsequent I2C calls work
    if (ret == ESP_FAIL)
    {
...
static lv_indev_t *encoder_indev = NULL;  // Encoder input device for volume control
...
static esp_err_t bsp_i2c1_bus_init(void)
{
    ...
    {
        ESP_LOGW(TAG, "I2C%d already installed; continuing", BSP_TOUCH_I2C_NUM);
        ret = ESP_OK;
    }
    if (ret != ESP_OK)
        return ret;
...
static esp_err_t bsp_spi_bus_init(void)
{
    ...
    const spi_bus_config_t qspi_cfg = {
        ...
        // Limit DMA transfer size to the LVGL draw buffer height to avoid SPI queue overruns.
        .max_transfer_sz = DRV_LCD_H_RES * LVGL_DRAW_BUFF_HEIGHT * DRV_LCD_BITS_PER_PIXEL / 8,
    };
...
static esp_err_t bsp_lcd_panel_init(void)
{
    ...
    const esp_lcd_panel_io_spi_config_t io_config = {
        ...
        .trans_queue_depth = 4,  // Increased from 2 to handle concurrent draw operations
...
static lv_disp_t *bsp_lvgl_init(void)
{
    ...
    const lvgl_port_display_cfg_t disp_cfg = {
        ...
        .flags = {
            .buff_dma = false,
            .buff_spiram = true,  // Match factory firmware - use SPIRAM for display buffer
        },
    };
...
/**
 * Initialize the wheel knob as an LVGL encoder input device.
 * This enables button callbacks via bsp_set_btn_* functions.
 */
static lv_indev_t *bsp_knob_indev_init(lv_disp_t *disp)
{
    ESP_LOGI(TAG, "Initialize knob input device for button detection");

    static const knob_config_t knob_cfg = {
        .default_direction = 0,
    ...
    lv_indev_t *indev = lvgl_port_add_encoder(&encoder);
    if (indev == NULL) {
        ESP_LOGE(TAG, "Failed to add encoder input device");
    } else {
        ESP_LOGI(TAG, "Encoder input device added successfully");
    }
...
static esp_codec_dev_handle_t bsp_audio_codec_speaker_init(void)
{
    ...
    ESP_LOGI(TAG, "Creating GPIO interface for ES8311...");
    const audio_codec_gpio_if_t *gpio_if = audio_codec_new_gpio();
    ESP_LOGI(TAG, "GPIO interface: %p", (void*)gpio_if);
    ...
    ESP_LOGI(TAG, "Creating I2C ctrl for ES8311 at port=%d, addr=0x%02x", i2c_cfg.port, i2c_cfg.addr);
    const audio_codec_ctrl_if_t *i2c_ctrl_if = audio_codec_new_i2c_ctrl(&i2c_cfg);
    ESP_LOGI(TAG, "I2C ctrl interface: %p", (void*)i2c_ctrl_if);
    if (i2c_ctrl_if == NULL) {
        ESP_LOGE(TAG, "Failed to create I2C ctrl for ES8311!");
        return NULL;
    }
...
static esp_codec_dev_handle_t bsp_audio_codec_microphone_init(void)
{
    ...
    ESP_LOGI(TAG, "Checking for microphone codec at 0x%02x and 0x%02x...", DRV_ES7243_I2C_ADDR, DRV_ES7243E_I2C_ADDR);
    esp_err_t check_13 = bsp_i2c_check(i2c0_port, DRV_ES7243_I2C_ADDR);
    esp_err_t check_14 = bsp_i2c_check(i2c0_port, DRV_ES7243E_I2C_ADDR);
    ESP_LOGI(TAG, "I2C check 0x13: %s, 0x14: %s", esp_err_to_name(check_13), esp_err_to_name(check_14));

    if (check_13 == ESP_OK)
    {
        ESP_LOGI(TAG, "Found ES7243 at 0x13, creating I2C ctrl...");
        audio_codec_i2c_cfg_t i2c_cfg = {
            .port = BSP_GENERAL_I2C_NUM,
        ...
        const audio_codec_ctrl_if_t *i2c_ctrl_if = audio_codec_new_i2c_ctrl(&i2c_cfg);
        ESP_LOGI(TAG, "ES7243 I2C ctrl: %p", (void*)i2c_ctrl_if);
        es7243_codec_cfg_t es7243_cfg = { .ctrl_if = i2c_ctrl_if };
        es7243_dev = es7243_codec_new(&es7243_cfg);
    }
    else if (check_14 == ESP_OK)
    {
        ESP_LOGI(TAG, "Found ES7243E at 0x14, creating I2C ctrl...");
        audio_codec_i2c_cfg_t i2c_cfg = {
            .port = BSP_GENERAL_I2C_NUM,
        ...
        const audio_codec_ctrl_if_t *i2c_ctrl_if = audio_codec_new_i2c_ctrl(&i2c_cfg);
        ESP_LOGI(TAG, "ES7243E I2C ctrl: %p", (void*)i2c_ctrl_if);
...
// Use static to avoid conflict with sensecap-watcher's bsp_io_expander_init
static esp_io_expander_handle_t board_io_expander_init_local(void)
{
...
void board_init()
{
    ...
    // Use sensecap-watcher's bsp_io_expander_init() so the static io_exp_handle is set correctly.
    // This prevents knob button init from calling bsp_io_expander_init() again and causing a power glitch
    // that resets the touch controller into a bad state.
    ...
    if (bsp_lcd_panel_init() == ESP_OK)
    {
        ...
        if (disp != NULL)
        {
            // Initialize encoder input device BEFORE registering button callbacks
            encoder_indev = bsp_knob_indev_init(disp);
    ...
    if (play_dev_handle && record_dev_handle)
    {
        ...
        // Note: Initial volume is set by volume_control_init() below
    }

    // 3. Initialize Touch (Optional for startup, but nice to have ready)
    bsp_touch_init();

    // 4. Initialize volume control with encoder wheel support
    // This sets the initial volume and registers wheel callbacks
    if (encoder_indev != NULL) {
        volume_control_init(encoder_indev);
    } else {
        ESP_LOGW(TAG, "Encoder not available, volume control disabled");
    }
